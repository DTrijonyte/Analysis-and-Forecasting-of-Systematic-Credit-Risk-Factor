 ...
 # Description:   - Cross-validation of the model, pls approach
 ...
 pls_data <- cbind.data.frame(z = z_df, pr_data)                                            
 pls_fit <- plsr(z ~., data = pls_data, cnomp = 1)                                                                                      #-------------ADDED
 ...
 colnames(eci_all) <- colnames(fit_macro_all) <- colnames(fit_total_all) <- colnames(b_all) <-
 colnames(fit_macro_all) <- colnames(fit_total_all) <- colnames(u_all) <- colnames(l_all) <-
