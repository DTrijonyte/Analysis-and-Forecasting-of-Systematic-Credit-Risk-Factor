 ...
 # Description:   - Cross-validation of the model
 ...
 eci_all <- coef_all <- se_all <- LB_all <- fit_macro_all <- fit_total_all <- 
 b_all <- u_all <- l_all <- accuracy_all <- accuracy_z1y <- accuracy_z2y <- 
 accuracy_z3y <- ps_selected_all <- eci_weights_all <- NULL
 ...
 ps_selected <- prescreened_features$ps_cor  # probably use just ps_cor sub-function of pre-screening     
 ps_selected_all <- rbind(ps_selected_all, ps_selected)
 ...
 pr_data <- combined_data_subset[, ps_selected]
 starting_nas <- apply(pr_data, 2, function(x)(length(x) - length(na.trim(x,"left")))) %>% max()
 pr_data %>% na.omit() %>% prcomp() -> pr_out
 direction <- (-1)*sign(pr_out$rotation[1, 1]) # the first variable is URX and its lags
 eci_weights <- direction*pr_out$rotation[,1]
 #print(eci_weights)
 eci_weights_all <- rbind(eci_weights_all, eci_weights) 
 # eliminate variable with not logical direction of impact
 z_df <- data.frame(z = window(z, end = cv_ends[index,]))
 eci_raw <- as.matrix(combined_data[dates >= dates[as.numeric(rownames(pr_data)[1])], 
 ...
 eci_all <- ts.union(eci_all, eci)
 ze_data <- cbind.data.frame(z = z_df, eci = eci[1:nrow(z_df)])
 eci_model <- Arima(ze_data$z, order = error_order, include.mean = FALSE, 
 xreg = as.matrix(ze_data[, "eci", drop = F]))
 coef_all <- rbind(coef_all, coef(eci_model))
 se_all <- rbind(se_all, sqrt(diag(eci_model$var.coef))) # for statistical validity
 LB_all <- c(LB_all, Box.test(zoo::na.approx(resid(eci_model)), fitdf = df, 
 lag = df + 3, type = "Ljung-Box")$p.value)

 fit_macro <- z - residuals(eci_model, type="regression")  # shows the macro fitted part
 fit_total <- z - residuals(eci_model, type="innovation")
 fit_macro_all <- ts.union(fit_macro_all, fit_macro)
 fit_total_all <- ts.union(fit_total_all, fit_total)
 
 b <- forecast_oos(eci_model, H, z_main = z, features_data = 
 data.frame(dates =  dates[dates >= tsp(eci)[1]], eci = eci), do_plot = F)
 ...
 colnames(eci_all) <- colnames(fit_macro_all) <- colnames(fit_total_all) <- 
 colnames(b_all) <- colnames(u_all) <- colnames(l_all) <-
 rownames(ps_selected_all) <- apply(cv_ends, 1, paste0, collapse = "Q")
 ...
 eci_coef <- coef_all[index, ncol(coef_all)]
 macro_contributions <- eci_coef*eci_weights_all[index,]
 
 total_data <- t(t(pr_data) * macro_contributions/s)
 bar_data <- ts.union(errors_all[,index], fit_inertia_all[,index], -m/s*eci_coef, total_data)
 colnames(bar_data) <- c("errors", "inertia", "scaling bias", colnames(total_data))
 ...
 # All ECI values, fluctuations at the ends are expected
 eci_plots <- autoplot(eci_all, xlab = "", ylab = "", color = col.custom[1], alpha = 0.1) +
 geom_hline(yintercept = 0, color = "black", size = 0.5)
 plot_list[["eci"]] <- eci_plots
 ...
 # ECI weights checks
 eci_weights_all %>% ts() %>% autoplot() -> eci_weights_plot
 plot_list[["weights"]] <- eci_weights_plot
 ...
 #print(sum(is.nan(se_all)))
 #sum(is.nan(coef_all))
 cat("\n\nCheck percent insignificant t statistics just for ECI:\n")
 print(sum(abs(t_all[,"eci"]) < 2, na.rm = T) / nrow(t_all) * 100) 
 
 cat("\n\nStability of residuals Ljung-Box assessment:\n")
 ...
 ps_selected = ps_selected_all,
 eci_weights = eci_weights_all,
 plots = plot_list))
